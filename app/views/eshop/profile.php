...
<?php if(is_object($profile_data)): ?>
	...
	<?php if(is_array($orders)):?>
			...
			<table class="table">
				<thead>
					<tr><th>Số thứ tự</th><th>Ngày đặt</th><th>Tổng cộng</th><th>Địa chỉ giao</th><th>Tỉnh/ Thành phố</th><th>Số điện thoại</th><th>Tình trạng</th><th>...</th></tr>
				</thead>
						...
						<tr style="position: relative;"><td><?=$order->id?></td><td><?=date("jS M Y H:i a",strtotime($order->date))?></td><td>$<?=$order->total?></td><td><?=$order->delivery_address?></td><td><?=$order->state?></td><td><?=$order->mobile_phone?><td><?=$status?></td>
							<td>
								...
								<div class="js-order-details details hide" >
									...
									<table class="table">
										<thead>
											<tr><th>Số lượng</th><th>Mô tả</th><th>Giá</th><th>Tổng cộng</th></tr>
										</thead>	
									...
									<h3 class="pull-right">Tổng kết: <?=$order->grand_total?></h3>
								</div>
							</td>
						</tr>
					<?php endforeach;?>
	...
	<?php else: ?>
		<h3 style="text-align: center;">Người dùng này chưa mua hàng</h3>
	<?php endif;?>

<?php else: ?>
	<h3 style="text-align: center;">Không tìm thấy profile</h3>
<?php endif;?>
