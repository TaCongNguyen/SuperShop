...
Class Slider
{
	...
	public function create($DATA,$FILES,$image_class = null)
	{
		...
		if(empty($arr['header1_text']) || !preg_match("/^[a-zA-ZÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂẾưăạảấầẩẫậắằẳẵặẹẻẽềềểếỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễệỉịọỏốồổỗộớờởỡợụủứừỬỮỰỲỴÝỶỸửữựỳỵỷỹ\s\W|_]+/", trim($arr['header1_text'])))
		{
			$this->error .= "Điền dữ liệu phù hợp cho (1) <br>";
		}

		if(empty($arr['header2_text']))
		{
			$this->error .= "Điền dữ liệu phù hợp cho (2)<br>";
		}

		if(empty($arr['text']))
		{
			$this->error .= "Điền dữ liệu phù hợp<br>";
		}

		if(empty($arr['link']))
		{
			$this->error .= "Điền link phù hợp<br>";
		}
		...
		if($this->error == ""){
			...
			foreach ($FILES as $key => $img_row) {
				...
				if($img_row['error'] == 0 && in_array($img_row['type'], $allowed))
				{
					...
					{
						$this->error .= $key . " size quá lớn<br>";
					}
				}
