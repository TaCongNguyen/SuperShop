...
Class Post
{
	...
	public function create($DATA,$FILES,$image_class = null)
	{
		...
		if(!preg_match("/^[a-zA-ZÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂẾưăạảấầẩẫậắằẳẵặẹẻẽềềểếỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễệỉịọỏốồổỗộớờởỡợụủứừỬỮỰỲỴÝỶỸửữựỳỵỷỹ\s\W|_]+/", trim($arr['title'])))
		{
			$_SESSION['error'] .= "điền tiêu đề hợp lệ <br>";
		}

		if(empty($arr['post']))
		{
			$_SESSION['error'] .= "Điền nội dung hợp lệ <br>";
		}
		...
		if($arr['image'] == ""){
			$_SESSION['error'] .= "Cần ít nhất 1 ảnh<br>";
		}
