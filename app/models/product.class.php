...
Class Product
{
	...
	public function create($DATA,$FILES,$image_class = null)
	{
		...
		if(!preg_match("/^[a-zA-ZÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂẾưăạảấầẩẫậắằẳẵặẹẻẽềềểếỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễệỉịọỏốồổỗộớờởỡợụủứừỬỮỰỲỴÝỶỸửữựỳỵỷỹ\s\W|_]+/", trim($arr['description'])))
		{
			$_SESSION['error'] .= " Hãy điền mô tả phù hợp !<br>";
		}

		if(!is_numeric($arr['quantity']))
		{
			$_SESSION['error'] .= "Hãy điền số lượng phù hợp !<br>";
		}

		if(!is_numeric($arr['category']))
		{
			$_SESSION['error'] .= "Hãy điền loại phù hợp!<br>";
		}
		
		if(!is_numeric($arr['brand']))
		{
			$_SESSION['error'] .= " Hãy điền thương hiệu phù hợp !<br>";
		}

		if(!is_numeric($arr['price']))
		{
			$_SESSION['error'] .= "Hãy điền giá cả phù hợp<br>";
		}
		...
		foreach ($FILES as $key => $img_row) {
			...
			if($img_row['error'] == 0 && in_array($img_row['type'], $allowed))
			{
				...
				{
					$_SESSION['error'] .= $key . " size quá lớn<br>";
				}
			}
	...
	public function edit($data,$FILES,$image_class = null)
	{
		...
		if(!preg_match("/^[a-zA-ZÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂẾưăạảấầẩẫậắằẳẵặẹẻẽềềểếỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễệỉịọỏốồổỗộớờởỡợụủứừỬỮỰỲỴÝỶỸửữựỳỵỷỹ\s\W|_]+/", trim($arr['description'])))
		{
			$_SESSION['error'] .= "Điền mô tả phù hợp cho sản phẩm !<br>";
		}
		...
		if(!is_numeric($arr['price']))
		{
			$_SESSION['error'] .= "Hãy điền giá phù hợp !<br>";
		}
