		...
		foreach ($POST as $key => $value) {
			...
			if($key == "country"){
				if($value == "" || $value == "-- Country --"){
					$this->errors[] = "Chọn một quốc gia hợp lệ";
				}
			}

			if($key == "state"){
				if($value == "" || $value == "-- State / Province / Region --"){
					$this->errors[] = "Chọn 1 tỉnh hợp lệ";
				}
			}

			if($key == "address1"){
				if(empty($value)){
					$this->errors[] = "Chọn địa chỉ hợp lệ";
				}
			}

			if($key == "postal_code"){
				if(empty($value)){
					$this->errors[] = "Chọn mã bưu điện hợp lệ";
				}
			}

			if($key == "mobile_phone"){
				if(empty($value)){
					$this->errors[] = "Chọn số điện thoại hợp lệ";
				}
			}
